...
import { getAcceptedTypes } from "../utils/acceptedTypes"
...
export const DropZone = ({ onContinue, isLoading }: DropZoneProps) => {
  const { translations, maxFileSize, dateFormat, parseRaw, title, acceptedFiles } = useRsi()
  ...
  return (
    ...
    >
      ...
      {isDragActive ? (
        ...
        <>
          <Text sx={styles.dropzoneText}>{translations.uploadStep.dropzone(title).title}</Text>
